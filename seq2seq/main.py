...
from data_utilz  import Sample, load_data, batchop, loss, accuracy
import importlib
...
if __name__ == '__main__':
    ...
    predict_parser.add_argument('-c', '--count', default=10, type=int, dest='count')
    predict_parser.add_argument('-l', '--length', default=10, type=int, dest='prediction_length')
    predict_parser.add_argument('-u', '--uniqueness', default=50, type=int,  dest='beam_width')
    predict_parser.add_argument('-g', '--gender', default='girl', type=str,  dest='gender')
    predict_parser.add_argument('-t', '--template', default='', type=str,  dest='template')
    predict_parser.add_argument('-f', '--force', default=False, action='store_true',  dest='teacher_force')
    ...
    train_feed     = DataFeed('train_feed',
                              dataset.trainset,
                              batchop    = _batchop,
    ...
    if args.task == 'predict':
        def predict(config, gender='', template='', teacher_force=False):
            input_ = None
            
            if args.template:
                sample = Sample('0', gender, tamil.utf8.get_letters(template), [])
                input_ = _batchop([sample], for_prediction=True)
                

            try:
                output = model.do_predict(input_= input_,
